...
![每个样式轮流出场,背景是一块真实的屏幕](docs/demo.gif)
...
路径永不重复 —— 只是幅度小到丢下去的文件还能落在你瞄的地方。光这一项就把光晕从每秒 0.25 拉到 13.5,最外圈从整整为零拉到 3.0。
...
因为那时 `time` 已经很大,缩放它会挪动整个图案。在 CPU 上积分,让速率可以自由变化而不留下可见的接缝。
