...
同一个想法的另一个项目 —— 一个能放在桌面上、会做引力透镜的黑洞。它同时还是一个
真正的回收站入口,而这一点是这个项目刻意不做的:在这里,丢文件进去只喂动画。
...
菜单里只有两个设置,因为需要的就这两个 —— 它们上面出现的任何东西,都是部件有话
要说,说完就消失:
...
- **它可以透镜活的屏幕**,这就是屏幕录制权限的用处 —— 但它不会自己开口要。全新
  安装时,它在透明背景上画洞和吸积盘,菜单里给出**让它弯曲身后的屏幕…**;只有你
  接受了,macOS 才会被问到。想关回去,在**高级…**的 Lens 一栏。如果你之前已经
  授权过,什么都不会变:部件会查一下,照旧带着透镜启动。
