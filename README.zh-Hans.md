...
下载最新的 **[`Black Hole.dmg`](../../releases/latest)**,或者自己构建:

```sh
./make-signing-cert.sh     # 只需一次 —— 见下面「权限」
./make-app.sh && open "dist/Black Hole.app"
```

需要 macOS 13+ 与 Apple 芯片。没有 Dock 图标,也没有主窗口:应用住在菜单栏里(⚫)。
菜单有简体中文、繁体中文和日语,跟随系统语言。

下载的这份有签名但**没有公证**,所以第一次打开要右键 ▸ **打开**,不能双击。
之后 macOS 就记住了。
...
- **往上面丢东西** —— 文件、一段选中的文字、一个链接、一张图 —— 然后看着它被
  吃掉(见下)。**它什么都不动**:动画就是全部,不会移动、复制或删除任何东西。
- **把指针放进去**,指针的影像会被拖向中心,拉长、变红,直到熄灭。真正的指针
  从头到尾没被碰过;移出来,影像就追上来。
- **它透镜的是活的屏幕**,这就是屏幕录制权限的用处。没有那个权限,它就在透明
  背景上画洞和星场,照常运行。
...
- **⌥⌘B** 隐藏和显示。用的是 Carbon 的 `RegisterEventHotKey`,刻意不用
  ...
  重启记得住。
- 位置是**按显示器**分别记的。停在笔记本屏幕角落的部件,没道理在你接上 5K 屏
  之后跑到它正中间去,反过来则会掉到屏幕外面。
...
洞自身的缩放也一样。它们唯一改变过的,是构图放弃使用窗口的多大一部分,也就是
说,它们的作用就是造出那圈死边。大小归窗口管,比例归 `HALO` 管。
...
这个应用是用自签名身份签的,不是 Developer ID,所以下载下来的 dmg 本身不能为
自己作保。让一次更新可以放心安装的,是 Sparkle 会拒绝任何 **EdDSA 签名**无法用
应用 `Info.plist` 里那把公钥验过的更新。私钥只在一个登录钥匙串里,别处没有 ——
丢了,就再也没有任何新版本能推送给已经装好的副本。

发布是一条命令:

```sh
./make-release.sh 1.1     # dmg、签名、appcast、打标签、GitHub release
```
...
所以让洞漂移。`DRIFT` 把它放回 GLSL 当年用来在整个终端里游走的那条李萨如曲线上
—— 每个轴两个不可通约的正弦,所以路径永不重复 —— 只是幅度小到丢下去的文件还能
落在你瞄的地方。光这一项就把光晕从每秒 0.25 拉到 13.5,最外圈从整整为零拉到 3.0。
...
有两个值得拥有的后果。丢一个文件下去,现在留下的是十秒钟的余韵,而不是两秒后
一无所有 —— 一次丢一把,你能看见这东西正在被喂。而且它回答了一个没人想起来问、
但所有人一看就懂的问题:盘是从哪儿来的。
...
因为那时 `time` 已经很大,缩放它会挪动整个图案。原版 GLSL 撞过同一堵墙,并留下
了警告。在 CPU 上积分,让速率可以自由变化而不留下可见的接缝。
...
一组追加,缩减填充位,永远不要重排。`./make-check.sh` 会逐字段比对两者,所以
重排会直接失败,而不是把后面每个字段悄悄喂给错误的参数。

## 与 Ghostty 那个着色器的差别

终端版本从外部驱动洞的大小和位置 —— 上下文窗口的填充度、一个番茄钟、一个从角落
长出来的漫游框。这些这里都没有:部件是你放好的一个固定物体,所以 `level`、漫游框
和工作区遮挡都去掉了。李萨如漂移留下了,干的是另一件活 —— 透镜光晕是背后那片
屏幕的静态映射,所以没有东西去动弯曲场的话,外圈根本不会变。留下来的是物理和
观感。

光标颜色编码也去掉了。那整套机制的存在,只是因为终端着色器拿不到自定义 uniform。
