...
**用 Windows?** [BlackHoleTrash](https://github.com/rrrjqy66/BlackHoleTrash) 是
同一个想法的另一个项目 —— 一个能放在桌面上、会做引力透镜的黑洞,同时还是一个真正
的回收站入口。这个项目你要的话也能这么用,但默认不是:开箱状态下,丢文件进去
只喂动画。
...
菜单是四样东西,它们上面出现的任何东西,都是部件有话要说,说完就消失:

- **透镜** —— 身后的屏幕**弯不弯**。两种都看得见你的屏幕:不弯的时候,部件在洞
  周围就是透明的,你看到的是真东西、没有变形,而且什么都不录。要弯,就得先录。
  见[权限](#权限)。
- **尺寸** —— 小、中、大。**抓着洞拖** —— 中间那块纯黑的圆,不是外面那圈光。
  洞以外的地方点击穿透,所以部件压在窗口上时不会抢走本该落在窗口上的点击。
- **样式** —— Inferno、Gargantua、M87\* donut、Face-on ember、Quasar、Blazar、
  Pure lens、Zen。**Gargantua** 就是《星际穿越》里那个,连电影故意偏离物理的
  三处也一并还原:多普勒不对称被关掉、吸积盘有厚度而不是一张刀片、眩光是横向
  的。它们对吸积盘该有多宽的意见差得很远(Gargantua 是 9 r_s,
  Blazar 是 16),所以洞会自动缩小,直到发亮的部分刚好装进画框。每个样式都填满
  同一个部件,谁也不会被裁掉。
- **轮换**让它每 **5 分钟**、**30 分钟**、**1 小时**或**1 天**换到下一个样式,
  在你勾选的那些样式里转。
  部件会淡出、换掉、再淡回来 —— 样式之间的分歧太大,硬切看起来只会像故障。
  当下该是哪个样式是从时钟算出来的,而不是数出来的,所以睡了八小时的 Mac 醒来
  直接落在该在的那个上,不会把错过的八次补着跳一遍。刚打开时从你勾选的第一个开始。手动点一个样式,或者动一下
  滑块,轮换就关掉 —— 否则到了下一个整点,你的选择会被拿走,而屏幕上没有任何
  东西解释为什么。

其余的事情,它不用你吩咐就在做:

- **往洞里丢东西** —— 文件、一段选中的文字、一个链接、一张图 —— 然后看着它被吃掉
  ([怎么吃的,以及为什么这么吃](#被吃掉))。**默认它什么都不动**:动画就是全部,
  不会移动、复制或删除任何东西。**丢进来的文件 ▸ 移到废纸篓**会让它变成真的 ——
  开启前会问一次,文件真正被移走的那一刻播放访达的废纸篓音效,而且只要这个模式
  开着,菜单栏那颗黑洞就带一个废纸篓角标,因为两种模式的动画长得一模一样。
  被移动的永远只有文件;文字、链接和图片仍然只是画出来而已。
- **把指针放进去**,指针的影像会被拖向中心,拉长、变红,直到熄灭。真正的指针从头
  到尾没被碰过;移出来,影像就追上来。
- **它可以透镜活的屏幕**,这就是屏幕录制权限的用处 —— 但它不会自己开口要。全新
  安装时它不弯曲屏幕,这既不花代价也不遮挡什么:该出现 warp 的地方部件是透明的,
  你的桌面就在那儿,只是前面多了一个不透明的洞和一个半透明的盘。选
  **透镜 ▸ 弯曲身后的屏幕**才是 macOS 被问到的那一刻。菜单会提一次,以免你根本
  不知道自己错过了什么;你一旦做出选择就不再提 —— 包括选择不弯。如果你之前已经授权过,什么都不会变:部件会查一下,照旧带着
  透镜启动。
...
- **检查更新…** —— 见[更新](#更新)。
- **关于 Black Hole** —— 系统自带的关于面板:版本、构建号和许可证,来自
  `Info.plist`。
...
**屏幕录制**是活屏幕透镜需要的,而在你打开那个透镜之前,它永远不会被请求。除此
之外什么都不要 ——
不要辅助功能,也不要输入监控。
